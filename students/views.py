...
def student_list(request):
    if not (request.user.is_admin or request.user.is_registrar or request.user.is_teacher):
    ...
    students = Student.objects.select_related('grade_level', 'section', 'school_year').all()
    ...
    if request.headers.get('HX-Request'):
        return render(request, 'students/partials/student_table.html', {
            'students': students
        })
...
def student_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('HX-Request'):
                return HttpResponse(
                    '<script>htmx.location="/students/";</script>',
...
def student_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        has_validated = Grade.objects.filter(student=student, status='validated').exists()
        if has_validated:
            messages.warning(request, f'Student {full_name} has validated grades. Deleting will remove all grade records.')
        student.delete()
        AuditLog.objects.create(
            user=request.user,
            action='delete',
...
def student_detail(request, pk):
    ...
    grades = Grade.objects.filter(student=student).select_related(
        'subject', 'grading_period', 'school_year'
